...
describe TaggableModel do
  ...
  context "setting tag list" do
    it "should be able to use two separate tags" do
      ...
      model.tag_list = {tag_names: tag_names}
    ...
    it "should set a tagger if passed in" do
      model = TaggableModel.new(name: "post")
      tagger = User.create(username: "Garrett")
      tag_names = "Sports, Food"
      model.tag_list = {tag_names: tag_names, tagger: tagger}
      model.save

      expect(HasTags::Tagging.last.tagger).to eq(tagger)
    end
